...
# We will update the latest versions of the functions.
# Based on the inspection, the latest 'update_property_safe' (OID 24606) uses arrays for owners/managers.
# We should target that one.
# For 'create_property_safe', we should also target the one with arrays if it exists, or the most complex one.
# Let's assume the user wants the most feature-rich one.
...
CREATE OR REPLACE FUNCTION public.create_property_safe(
    ...
    p_gate_code text, 
    p_closet_code text, 
    p_wifi_network text,    -- NEW
    p_wifi_password text,   -- NEW
...
BEGIN
    ...
    SET name = p_name, display_address = p_address, hcp_customer_id = p_hcp_cust, hcp_address_id = p_hcp_addr, 
        ...
        wifi_network = p_wifi_network, wifi_password = p_wifi_password -- NEW
    WHERE id = p_id;
