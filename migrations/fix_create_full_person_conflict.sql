-- Drop all conflicting versions of create_full_person
DROP FUNCTION IF EXISTS public.create_full_person(text, text, text, text, text, uuid, uuid);
DROP FUNCTION IF EXISTS public.create_full_person(text, text, text, text, uuid, text, uuid);

-- Recreate with correct signature (p_role after p_tenant_id with default)
