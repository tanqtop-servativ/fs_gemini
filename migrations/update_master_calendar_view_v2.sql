-- Update master_calendar view to exclude deleted service opportunities
-- This fixes the issue where deleted/archived items still appear in the calendar
...
WHERE so.status != 'archived' AND so.deleted_at IS NULL;
