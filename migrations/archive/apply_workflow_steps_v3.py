...
def apply_migration():
    ...
    try:
        ...
        with open('migrations/create_service_workflow_steps_v3.sql', 'r') as f:
            sql = f.read()
            
        cur.execute(sql)
        conn.commit()
        print("Successfully created/updated service_workflow_steps table.")
        
        # Verify
        cur.execute("SELECT column_name FROM information_schema.columns WHERE table_name='service_workflow_steps'")
