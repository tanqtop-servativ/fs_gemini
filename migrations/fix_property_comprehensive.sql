-- Create master_item_catalog if it doesn't exist
...
-- Seed some catalog items if empty
INSERT INTO master_item_catalog (item_name, category)
SELECT 'Toilet Paper', 'Consumable'
WHERE NOT EXISTS (SELECT 1 FROM master_item_catalog);

INSERT INTO master_item_catalog (item_name, category)
SELECT 'Paper Towels', 'Consumable'
WHERE NOT EXISTS (SELECT 1 FROM master_item_catalog WHERE item_name = 'Paper Towels');

INSERT INTO master_item_catalog (item_name, category)
SELECT 'Hand Soap', 'Consumable'
WHERE NOT EXISTS (SELECT 1 FROM master_item_catalog WHERE item_name = 'Hand Soap');


-- Drop ambiguous functions to be safe
...
-- Recreate create_property_safe with EXACT signature
...
-- Recreate update_property_safe with EXACT signature
