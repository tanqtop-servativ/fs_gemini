...
BEGIN
    ...
    IF p_manager_ids IS NOT NULL THEN
        FOREACH pid IN ARRAY p_manager_ids LOOP
            -- Check if already assigned as Owner? No, allow dual assignment if needed, OR just insert as Manager.
            -- If user is both, they will have two rows? 
            -- Constraint is PK (property_id, person_id). So one person can only have ONE role per property?
            -- If so, we need to decide precedence. Usually Owner > Manager.
            -- BUT, if I want to be both, I can't.
            -- Let's assume for now we just insert. If PK violation, we skip.
            -- Wait, if PK is (property_id, person_id), then a person can only have ONE role per property.
            -- This simplifies things. If I am an Owner, I don't need to be a Manager.
