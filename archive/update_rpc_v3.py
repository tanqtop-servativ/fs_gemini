...
CREATE OR REPLACE FUNCTION public.create_property_safe(
    ...
    p_has_casita boolean,       -- NEW
    p_casita_code text,         -- NEW
    p_square_footage integer,   -- NEW
    p_feeds jsonb DEFAULT '[]'::jsonb, 
    p_inventory jsonb DEFAULT '[]'::jsonb,
    p_attachments jsonb DEFAULT '[]'::jsonb -- NEW
)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    VALUES (
        ...
        p_has_casita, p_square_footage -- NEW
    )
    ...
    IF p_casita_code != '' THEN INSERT INTO property_access_codes (property_id, code_type, code_value) VALUES (new_prop_id, 'Casita', p_casita_code); END IF; -- NEW
    ...
    -- Attachments (NEW)
    -- We assume attachments are uploaded and we just get metadata here. 
    -- Actually, for attachments, it's better to handle them like reference photos (direct insert/delete from UI)
    -- BUT if we want to support "save all at once" logic, we can do it here.
    -- However, the prompt says "Add the ability to attach any kind of file".
    -- Usually file uploads are async. Let's stick to the pattern used for Reference Photos (async upload/delete).
    -- So we might NOT need to handle attachments in this RPC if we do it purely async in UI.
    -- BUT, let's support passing them in case we want to update metadata.
    -- For now, I will NOT put attachment logic in the main RPC to avoid complexity, 
    -- I will handle attachments in the UI with direct Supabase calls like Reference Photos.
    -- Wait, the prompt implies "update the display/edit modals".
    -- I'll stick to async for attachments.
END;
$function$;
"""
...
CREATE OR REPLACE FUNCTION public.update_property_safe(
    ...
    p_feeds jsonb DEFAULT '[]'::jsonb, 
    p_inventory jsonb DEFAULT '[]'::jsonb
    -- p_attachments not needed if handled async
)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    SET name = p_name, display_address = p_address, hcp_customer_id = p_hcp_cust, hcp_address_id = p_hcp_addr, 
        ...
        has_casita = p_has_casita, square_footage = p_square_footage -- NEW
    WHERE id = p_id;
    ...
    IF p_casita_code != '' THEN INSERT INTO property_access_codes (property_id, code_type, code_value) VALUES (p_id, 'Casita', p_casita_code); END IF; -- NEW
