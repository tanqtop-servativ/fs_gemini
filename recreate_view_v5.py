...
# Joins property_assignments (pa) directly to roles (r) via pa.role_id
# This respects the per-property role assignment.
sql_create = """
 ...
 SELECT p.id,
    ...
    ( SELECT string_agg((pe.first_name || ' '::text) || COALESCE(pe.last_name, ''::text), ', '::text) AS string_agg
           FROM property_assignments pa
             ...
             JOIN roles r ON r.id = pa.role_id -- CHANGED: Join on pa.role_id
